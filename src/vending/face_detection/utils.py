...
def od_collate_fn(batch: list) -> tuple[torch.Tensor, list[torch.FloatTensor]]:
...
def encode(
    matched: torch.Tensor, priors: torch.Tensor, variances: list
) -> torch.Tensor:
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: list) -> tuple:
        ...
        conf_t_label = torch.zeros(
            (num_batch, num_dbox), dtype=torch.int64, device=self.device
        )
        loc_t = torch.zeros(
            (num_batch, num_dbox, 4), dtype=torch.float32, device=self.device
        )
        ...
        loss_c = F.cross_entropy(
            batch_conf,
            torch.clamp(conf_t_label.view(-1), 0, num_classes - 1),
            reduction="none",
        )
        ...
        loss_c = F.cross_entropy(
            conf_hnm, torch.clamp(conf_t_label_hnm, 0, num_classes - 1), reduction="sum"
        )
